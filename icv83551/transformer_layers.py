...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        ############################################################################
        # Construct the positional encoding array as described in            #
        ...
        position = torch.arange(max_len).unsqueeze(1) # Position col
        div_term = torch.pow(10000.0, -torch.arange(0, embed_dim, 2) / embed_dim) # What we do division with
    ...
    def forward(self, x):
        ...
        ############################################################################
        # Index into your array of positional encodings, and add the         #
        # appropriate ones to the input sequence. Don't forget to apply dropout    #
        # afterward. This should only take a few lines of code.                    #
        ############################################################################
        
        current_pe = self.pe[:, :S, :].to(x.device)
        output = self.dropout(x + current_pe)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # Second part mult
        scores = (Q @ K.transpose(-2, -1)) / math.sqrt(H_dim)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, memory, tgt_mask=None):
        ...
        ############################################################################
        # Complete the decoder layer by implementing the remaining two       #
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        ############################################################################
        # Divide the image into non-overlapping patches of shape             #
        ...
        x = x.reshape(N, C, H // patches, patches, W // patches, patches) # separate the patch dimensions
        x = x.permute(0, 2, 4, 1, 3, 5) # permute like was asked
        x = x.reshape(N, self.num_patches, self.patch_dim) # flatten
        out = self.proj(x)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, src_mask=None):
        ...
        """
        ############################################################################
        # Implement the encoder layer by applying self-attention followed    #
