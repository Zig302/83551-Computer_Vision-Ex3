...
class Unet(nn.Module):
    def __init__(
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            down_block = None
            ##################################################################
            # Create one UNet downsampling layer `down_block` as a ModuleList.
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out_ups):
            up_block = None
            ##################################################################
            # Create one UNet upsampling layer as a ModuleList.
    ...
    def forward(self, x, time, model_kwargs={}):
        ...
        ##################################################################
        # Process `x` through the U-Net conditioned on the context.
        #
        # 1. Downsampling:
        ...
        for down_block in self.downs:
            ...
            for block in down_block[:-1]: # Loop through all ResNet blocks
                x = block(x, context)
                layer_skips.append(x)
            
            # Save the skips for this specific depth level
            skip_connections.append(layer_skips)
            
            # Apply the final Downsample layer
            x = down_block[-1](x)
        ...
        # Upsampling
        for up_block, layer_skips in zip(self.ups, reversed(skip_connections)):
            x = up_block[0](x)
            
            # LIFO
            resnet_blocks = up_block[1:]
            skips_reversed = reversed(layer_skips)
            
            for block, skip in zip(resnet_blocks, skips_reversed):
