...
def get_similarity_no_loop(text_features, image_features):
    ...
    similarity = None
    ############################################################################
    # Compute the cosine similarity. Do NOT use for loops.               #
    ############################################################################
    ...
    # Normalize features
    normalized_texts = text_features / text_norms
    normalized_images = image_features / image_norms
    
    # Matrix mult
    similarity = normalized_texts @ normalized_images.T
...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    ############################################################################
    # Find the class labels for images.                                  #
    ############################################################################
    ...
    pred_classes = [class_texts[i] for i in torch.argmax(sims, axis=0)] # select classes based max vals
...
class CLIPImageRetriever:
    ...
    def __init__(self, clip_model, clip_preprocess, images, device):
        ...
        """
        ############################################################################
        # Store all necessary object variables to use in retrieve method.    #
        ...
        self.image_features = self.clip_model.encode_image(images_tensor) # Encode
        
        # Normalize
        image_norms = torch.linalg.norm(self.image_features, dim=1, keepdim=True)
        self.image_features = self.image_features / image_norms
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        top_indices = []
        ############################################################################
        # Retrieve the indices of top-k images.                              #
        ############################################################################
        ...
        # take the top k values and their indx
...
class DINOSegmentation:
    def __init__(self, device, num_classes: int, inp_dim : int = 384):
        ...
        ############################################################################
        # Define a very lightweight pytorch model, optimizer, and loss       #
    ...
    def train(self, X_train, Y_train, num_iters=500):
        ...
        """
        ############################################################################
        # Train your model for `num_iters` steps.                            #
        ############################################################################
        
        self.model.train()
        for i in range(num_iters):
            self.optimizer.zero_grad() # Clear gradients 
            logits = self.model(X_train) # Forward pass
            loss = self.criterion(logits, Y_train) # Loss
    ...
    def inference(self, X_test):
        ...
        # Forward pass to get the scores for each class
        logits = self.model(X_test)
        
        # Find the index of the highest score for each patch
