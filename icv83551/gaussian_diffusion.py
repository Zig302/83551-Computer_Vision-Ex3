...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample(self, x_t, t: int, model_kwargs={}):
        ...
        model_pred = self.model(x_t, t, model_kwargs)
        if self.objective == "pred_noise":
            x_start = self.predict_start_from_noise(x_t, t, noise=model_pred)
        else:
            x_start = model_pred
            
        x_start = x_start.clamp(-1.0, 1.0)
        model_mean, posterior_std = self.q_posterior(x_start=x_start, x_t=x_t, t=t)
        if t[0] > 0:
            noise = torch.randn_like(x_t)
        else:
            noise = 0.0
            
        x_tm1 = model_mean + posterior_std * noise  
    ...
    def p_losses(self, x_start, model_kwargs={}):
        ...
        model_pred = self.model(x_t, t, model_kwargs) # Predict
        squared_error = (model_pred - target) ** 2
        loss = (loss_weight * squared_error).mean()
