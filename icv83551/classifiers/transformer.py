...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        ############################################################################
        # Implement the forward function for CaptionTransformer.             #
        # A few hints:                                                             #
        ...
        caption_embeddings = self.embedding(captions) # embedding
        caption_embeddings = self.positional_encoding(caption_embeddings) # pos encoding
        
        image_embeddings = self.visual_projection(features).unsqueeze(1) # embedding
        tgt_mask = torch.tril(torch.ones((T, T), device=features.device)) # masking
        
        # outputs
        decoder_output = self.transformer(caption_embeddings, image_embeddings, tgt_mask=tgt_mask)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        # convert the input image into a sequence of patch vectors
